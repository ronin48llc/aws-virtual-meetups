 ...
 * Get GitHub token from Secrets Manager.
 *
 * Issue #117: the CDK template stores the secret as a JSON object —
 *   { "token": "<value>", "placeholder": "<auto-generated>" }
 * — produced by `generateSecretString.secretStringTemplate`. The previous
 * implementation returned the whole JSON string verbatim, which turned the
 * Authorization header into
 *   `token {"token":"...","placeholder":"..."}`
 * and GitHub returned 401 for every publication. An operator who manually
 * overwrites the secret with a plain string also needs to work — hence
 * the fallback path.
 */
async function getGitHubToken() {
  ...
  if (typeof raw !== 'string' || raw.length === 0) {
    throw new Error('GitHub token secret is missing or empty');
  }

  // Prefer the JSON-template format. Fall through to plain-string if the
  // secret isn't JSON or doesn't have a usable `token` field.
  try {
    const parsed = JSON.parse(raw);
    if (parsed && typeof parsed === 'object' && typeof parsed.token === 'string' && parsed.token.length > 0) {
      return parsed.token;
    }
  } catch (_jsonErr) {
    // Not JSON — treat raw as the token string.
  }
  return raw;
}
